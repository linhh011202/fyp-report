...
The Gateway Dashboard project integrates two critical external systems: user authentication through Single Sign-On (SSO) providers, and payment processing for API usage billing. This chapter examines the technology decisions for these integrations.

Section~\ref{sec:sso_selection} compares popular SSO authentication providers available in the market, analyses their strengths and user base coverage, and explains the rationale for selecting Google and Apple. Section~\ref{sec:payment_selection} evaluates the two leading payment processing platforms, Stripe and PayPal, and justifies the selection of Stripe based on subscription support, developer experience, and webhook reliability. Finally, Section~\ref{sec:tech_decisions_summary} consolidates the key technology decisions and their justifications.

\section{Authentication: SSO Provider Selection}
\label{sec:sso_selection}
...
\begin{itemize}
    \item \textbf{Subscription native Design:} The Gateway Dashboard monetizes through usage based subscriptions (API request quotas, tiered pricing). Stripe's billing system natively supports this model with prorated upgrades, quota management, and automated renewal. These are features that PayPal does not offer cleanly. Implementing these on PayPal would require custom workarounds.
    ...
    \item \textbf{Audience Alignment:} The Gateway Dashboard's users are software developers. Developers prioritize technical functionality, API quality, and developer friendly tooling, which is Stripe's core strength, over consumer familiarity with brand names like PayPal.
\end{itemize}
...
\label{sec:tech_decisions_summary}
