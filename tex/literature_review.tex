...
Stripe Price Caching & Minimize API calls & Sub-ms retrieval & 1 hour \\ \hline
Token Blacklist & Immediate revocation & <1ms lookup & Token expiry \\ \hline
\end{tabular}
\end{table}
...
\begin{enumerate}
    ...
    \item \textbf{Security:} Enables JWT token blacklisting for immediate logout/revocation
...
Blacklisting & Redis-based & Immediate revocation \\ \hline
Cookie Storage & HttpOnly, Secure, SameSite & XSS/CSRF protection \cite{barth2008securing} \\ \hline
...
Caching & Redis & Performance (sub-ms) + security (blacklist) \\ \hline
Token Strategy & JWT + Redis blacklist & Stateless scalability + immediate revocation \\ \hline
\end{tabular}
\end{table}
